...
#CLD = getvar(wrf_file, "W", timeidx=time)
CLD1 = getvar(wrf_file1, "W", timeidx=time)
...
for y in level:
    ...
    #plt.contourf(lons, lats, CLD_DIFF,v, cmap=plt.get_cmap("bwr"),extend='max') ###use if conc does not cahnege much between plots
    ...
    #plt.title("W at level="+str(y),weight='bold',fontdict=font)
...
#plt.colorbar(shrink=.90)
    ...
    #cb.gridlines(color="black", linestyle="dotted")
    #plt.savefig("W AT LEVEL ="+str(y)+".png",dpi=300)
    #plt.show()
    ...
    v = np.linspace(-5,5,50) ##COLORBAR LIMITS
    ...
    plt.colorbar(ax=ax)
    plt.title("W(cm/s) at level="+str(y),weight='bold',fontdict=font)
