...
CLD = getvar(wrf_file, "QRAIN", timeidx=time)
CLD1 = getvar(wrf_file1, "QRAIN", timeidx=time)
...
for y in level:
    ...
    x = CLD[y,:,:]
    z = CLD1[y, :, :]
    CLD_DIFF = (x - z)*1000 #G/KG ###*100 FOR BETTER READABILITY
...
#plt.colorbar(shrink=.90)
    ...
    plt.title("QRAIN (g/kg) change at level="+str(y),weight='bold',fontdict=font)
    #plt.figure(figsize=(8,6))
    plt.savefig("QRAIN AT LEVEL ="+str(y)+".png",dpi=300)
    plt.show()