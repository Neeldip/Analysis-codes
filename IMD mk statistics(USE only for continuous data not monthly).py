...
#sys.stdout = open(r'F:\IMD\z_statistics_dec.txt', 'w')   #<----------------
import pymannkendall as pk

#*****************concatenate************************
#ncrcat * IMD.nc

ds = open_mfdataset(r"I:\observation_data\APRIL\DATA\Rainfall\IMD_gridded_datasets\RAW data\NETCDF\New folder\Working\apr_2011-2020.nc") #<----------------
'''
...
#df = pd.read_csv(r"F:\IMD\z_statistics_dec.txt",header=None)  #<----------------
#df = np.asarray(df)
#print(np.nanmax(df))
...
import h5py
ds = open_mfdataset(r"F:\IMD\tau_apr.nc") #<----------------
...
v = np.linspace(-0.001,0.001,20)
#plt.contourf(lon,lat,precp,v,cmap="nipy_spectral",orientation="vertical",
#             transform=cartopy.crs.PlateCarree())
plt.pcolormesh(lon,lat,precp,vmin=-0.001,vmax=0.001,transform=cartopy.crs.PlateCarree(),cmap="nipy_spectral")
plt.colorbar(ticks=v,shrink=0.93)
plt.title("April")  #<----------------
plt.savefig(r'F:\IMD\tau_apr.png',dpi=300,bbox_inches='tight')  #<----------------
plt.show()