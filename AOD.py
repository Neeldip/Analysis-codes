...
TAU400 = getvar(wrf, "TAUAER2", timeidx=time)
TAU600 = getvar(wrf, "TAUAER3", timeidx=time)
...
#v = np.linspace(np.min(x),np.max(x),30)
v = np.linspace(0,1,21)
plt.contourf(lons, lats, x, v,cmap=plt.get_cmap("jet"),extend='max',transform=cartopy.crs.PlateCarree()) ###use if conc does not cahnege much between plots
...
plt.savefig("F:\WRF-CHEM ANALYSIS NEW\AOD\AOD_april_10-19.png",dpi=600,bbox_inches='tight')
plt.show()
