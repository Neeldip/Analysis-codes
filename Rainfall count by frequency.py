...
fn = r'F:\WRF-CHEM ANALYSIS Chap 5\rainfall\RAIN_count_by_frequency_NOR-NOBCABS.nc'   #<----------------
...
for i in range(0,231,1):
    ...
    #    else:
            ...
            low_inc[i,j] = (np.size(z1) - np.size(z2))
            moderate_inc[i,j] = (np.size(z2) - np.size(z6))
            heavy_inc[i,j] = np.size(z6)
            low_dec[i,j] = (np.size(z3) - np.size(z4))
            moderate_dec[i,j] = (np.size(z4) - np.size(z7))
            heavy_dec[i,j] = (np.size(z7))
            no_change[i,j] = (np.size(z5))
            print(i)
            print(j)
