...
BIN1 = getvar(wrf_file, "num_a01", timeidx=time)
BIN2 = getvar(wrf_file, "num_a02", timeidx=time)
BIN3 = getvar(wrf_file, "num_a03", timeidx=time)
BIN4 = getvar(wrf_file, "num_a04", timeidx=time)

BIN_1=BIN1+BIN2+BIN3+BIN4
BIN1=None
BIN2=None
BIN3=None
BIN4=None

BIN5 = getvar(wrf_file, "num_a01", timeidx=time)
BIN6 = getvar(wrf_file, "num_a02", timeidx=time)
BIN7 = getvar(wrf_file, "num_a03", timeidx=time)
BIN8 = getvar(wrf_file, "num_a04", timeidx=time)
BIN_2=BIN5+BIN6+BIN7+BIN8
BIN5=None
BIN6=None
BIN7=None
BIN8=None
...
if time == ALL_TIMES:
    BIN_1 = BIN_1.mean(axis=0)
    BIN_2 = BIN_2.mean(axis=0)
    # CLD = CLD.std("Time")
    # CLD1 = CLD1.std("Time")
...
for y in level:
    ...
    x = BIN_1[y, :, :]
    z = BIN_2[y, :, :]
    CLD_DIFF =  ((x - z))#*1000#/10**7   #* 100  # 0000#*1000000000 ###*100 FOR BETTER READABILITY
    #ne=CLD_DIFF[123:203, 182:262]
    #ne_nosouth = CLD_DIFF[157:203, 182:262]
    #print(np.nanmean(ne))
    ...
    v = np.linspace(-100.0, 100.0, 21)
    plt.contourf(lons, lats, CLD_DIFF, v, cmap=plt.get_cmap("bwr"), antialiased=False,
    ...
    plt.savefig(r"F:\WRF-CHEM ANALYSIS NEW\WRF level analysis\num_cw\num_cw_NOR-NOBCABS_AT_LEVEL =" + str(y) + ".png",dpi=600, bbox_inches='tight')
    plt.show()
