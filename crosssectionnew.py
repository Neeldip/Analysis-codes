...
#print(ter)
BIN1 = getvar(wrf_file, "EXTCOF55", timeidx=time)
...
QCLOUD_contours = ax_cross.contourf(xs, ys, to_np(dbz_cross_filled),v, cmap="jet")
ax_cross.tick_params(labelsize=16,axis='both',which='both',bottom=False,labelbottom=False) ###yticks font
# Add the color bar
cb_dbz = fig.colorbar(QCLOUD_contours, ax=ax_cross,ticks=v)
#cb_dbz = fig.colorbar(QCLOUD_contours, ax=ax_cross)
cb_dbz.ax.tick_params(labelsize=19,axis = 'y')
...
# Add a title
#ax_cross.set_title("(a) ACM2-April ", {"fontsize": 16})
pyplot.savefig(r"F:\WRF-CHEM ANALYSIS\EXTCOEFF\EXTCOEFF_NOR_"+str(cross_start)+str(cross_end)+".png",dpi=600,bbox_inches='tight')
pyplot.show()
CONC=None
