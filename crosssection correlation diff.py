...
cross_end = CoordPair(lat=29.5, lon=92.0)
#time= ALL_TIMES
#Get the WRF variables
ht = getvar(wrf_file, "z", timeidx=0)
ter = getvar(wrf_file, "ter", timeidx=0)
#BIN1 = getvar(trad, "QCLOUD", timeidx=time)
#BIN2 = getvar(trad1, "QCLOUD", timeidx=time)
...
#X=None
#Y=None
z_cross = vertcross(DIFF, ht, wrfin=wrf_file,start_point=cross_start,end_point=cross_end,latlon=True, meta=True)
#z_cross.plot()
...
pyplot.savefig('F:\WRF-CHEM ANALYSIS\WRF level analysis\CORRELATION_CLDFRA_TEMP'+str(cross_start)+str(cross_end)+'.png',dpi=600)
pyplot.show()
